...
# LocalWeb 集群只使用显式选择的局域网 IPv4 地址。Windows 混合协议栈下，
# master/schedd 可能分别占用同一动态端口的 IPv6/IPv4 监听，导致 Shadow
# 的父会话被错误 daemon 拒绝（Server rejected our session id）。
ENABLE_IPV4 = TRUE
ENABLE_IPV6 = FALSE
PREFER_IPV4 = TRUE
UID_DOMAIN = {machine}
...
# 两台非域 Windows 主机之间 NTSSPI 可能无法完成模拟。客户端和 daemon
# 必须使用相同的回退方法，否则子节点无法向父 collector 发布 StartD 广告。
SEC_DEFAULT_AUTHENTICATION_METHODS = NTSSPI, CLAIMTOBE
SEC_READ_AUTHENTICATION_METHODS = NTSSPI, CLAIMTOBE
SEC_WRITE_AUTHENTICATION_METHODS = NTSSPI, CLAIMTOBE
SEC_DAEMON_AUTHENTICATION_METHODS = NTSSPI, CLAIMTOBE
SEC_CLIENT_AUTHENTICATION_METHODS = NTSSPI, CLAIMTOBE
SEC_ADMINISTRATOR_AUTHENTICATION_METHODS = NTSSPI, CLAIMTOBE
